...
for (let index = 0; index < basket.length; index += 1){
    let frutaAtual = basket[index];
    let repFrutaAtual = 0;
    for (let newIndex = 0; newIndex < basket.length; newIndex += 1){
        if (basket[newIndex] === frutaAtual){
            repFrutaAtual += 1;
            frutasBasket[frutaAtual] = repFrutaAtual;
        }
    }
}
console.log(frutasBasket);